...
def math_metric(
    ...
    def sample_level_fn(
        ...
        extracted_golds = [
            parse(gold, gold_extraction_target, parsing_timeout=None) for gold in golds
        ]
            ...
            aggregation_function(
                [
                    (
                        1.0
                        if any(
                            verify(gold, pred, precision, timeout_seconds=None)
        ...
        def wrapper(*args, **kwargs):
            ...
            if process.is_alive():
                ...
                raise TimeoutException(
                    f"timeout occurred while running the {fn.__name__} after {timeout_seconds} seconds"
                )
            ...
            try:
                ...
                raise RuntimeError("no result received from the process")
