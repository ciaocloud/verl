...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg', 'power_grpo'.
        clip_cov_ratio (float): Ratio of tokens to be clipped for clip-cov loss.
        clip_cov_lb (float): Lower bound for clip-cov loss.
        clip_cov_ub (float): Upper bound for clip-cov loss.
        kl_cov_ratio (float): Ratio of tokens to be applied KL penalty for kl-cov loss.
        ppo_kl_coef (float): KL divergence penalty coefficient.
        length_alpha (float): Length scaling exponent for power_grpo. 0.5 = sqrt(L), 0 = no scaling, 1 = full normalization.
    """
    ...
    length_alpha: float = 0.5  # For power_grpo: 0.5 = sqrt(L)
